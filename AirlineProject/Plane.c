...
//files functions
int readPlaneFromFile(FILE* fp, Plane* pPlane)
{
	if (!fp || !pPlane)
		return 0;

	if (!fread(pPlane, sizeof(Plane), 1, fp))
		return 0;

	return 1;
}

int writePlaneToFile(FILE* fp, const Plane* pPlane)
{
	if (!fp || !pPlane)
		return 0;

	if (!fwrite(pPlane, sizeof(Plane), 1, fp))
		return 0;
